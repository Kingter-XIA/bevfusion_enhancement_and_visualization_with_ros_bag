...
class MyListener:
    ...
    def points_callback(msg):
        ...
        if (data is not None):
            with torch.no_grad():
                output = MyListener.model(return_loss=False, rescale=True, **data)              #获取depth信息

                #发布推理结果
                MyListener.publish_bboxes(output)
                print(f'bboxes published')
