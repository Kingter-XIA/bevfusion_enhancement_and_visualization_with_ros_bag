...
class LoadMultiViewImageFromFiles:
    ...
    def __call__(self, results):
        ...
        #Add raw images into the dict for visualization         
        img_list = []
        for img in images:
            img_list.append(np.array(img))        
        results["rawimg"] = np.stack(img_list, axis=0)          #[6,900,1600,3]
...
class NuscenesLoadData():
    ...
    def get_data(self, index):
        if(index < self.data_infos_len):
            data = self.get_data_info(index)
            data = self.pre_pipeline(data)
...
def main():
    ...
    checkpoint  = './pretrained/epoch_13_cam_att_lidar_mAP6823.pth'
    #cfg_path    = './configs/nuscenes/det/transfusion/secfpn/camera+lidar/swint_v0p075/convfuser.yaml'
    #checkpoint  = './pretrained/bevfusion-det.pth'
    ...
    for i in range (LoadData.get_data_infos_len()):
        ...
        #modify image
        if(i%2):
            data['img'].data[0] = torch.full((1, 6, 3, 256, 704), 0.01, dtype=torch.float32)
       
        #modify points
        """
        N,_ = data['points'].data[0][0].shape 
        data['points'].data[0][0] = torch.full((N,5), 0.01, dtype=torch.float32)
        """
