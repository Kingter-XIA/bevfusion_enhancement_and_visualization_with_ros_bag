...
def load_feats(feat_path = g_feat_path, feat_name = g_feat_name):
    ...
    if 'lidar_depth_1_6_1_256_704.bin' in feat_name:
    ...
    elif 'lidar_gt_depth_1_6_1_32_88_3.bin' in feat_name:
        H,W,C,N=32,88,1,6
        ...
        print(f'Error!!!!----feat-name = {feat_name}')
    ...
    print(f'feat_name = {feat_name} mean max ={image1.max()} min={image1.min()}')
    image1[image1 >0.2] = 0.2

    #image2 取最大值
    image2 = image.max(axis=0)
    print(f'feat_name = {feat_name} max max ={image2.max()} min={image2.min()}')
    image2[image2 >1.8] = 1.8

    #image3取和
    image3 = image.sum(axis=0)
    print(f'feat_name = {feat_name} sum max ={image3.max()} min={image3.min()}')
...
def load_6_feats(feat_path = g_feat_path, feat_name = g_feat_name):
    ...
    if 'lidar_depth_1_6_1_256_704.bin' in feat_name:
        ...
        H,W,C,N=32,88,64,6
    elif 'lidar_gt_depth_1_6_1_32_88.bin' in feat_name:
        H,W,C,N=32,88,1,6
    elif 'img_depth_6_118_32_88.bin' in feat_name:
        H,W,C,N=32,88,118,6
    elif 'img_feat_6_80_32_88.bin' in feat_name:
    ...
    print(f'feat={feat_name} mean max ={image1.max()} min={image1.min()}')
    #image1[image1 >0.6] = 0.6
    print(f'image1 -------shape={image1.shape}')

    #image2 取最大值
    image2 = image.max(axis=1)
    print(f'feat={feat_name} max max ={image2.max()} min={image2.min()}')
    #image2[image2 >2.8] = 2.8

    #image3取和
    image3 = image.sum(axis=1)
    print(f'feat={feat_name} sum max ={image3.max()} min={image3.min()}')
    #image3[image3 >30.8] = 30.8
...
def show_imgs_feats():
    ...
    feat_name='img_feat_1_6_256_32_88.bin'
    ...
    feat_name ='lidar_gt_depth_1_6_1_32_88.bin'
    ...
    ###############################################################################
    # Subfigure-4: 6 features - mean
    ###############################################################################
    axes = fig4.subplots(6,1)                          # 1 rows, 6 columns
    bevs = load_bev_feats(feat_path=g_feat_path)
    for i in range (6):
        axes[i].imshow(bevs[i])
        axes[i].axis('off')
...
def main():
    global g_feat_path, g_feat_name

    # List all subfolders
    directory_path = './bev_feat'
    subfolders = [d for d in os.listdir(directory_path) if os.path.isdir(os.path.join(directory_path, d))]

    index = 0    
    while True:
        try:
            g_feat_path = os.path.join(directory_path, subfolders[index])
            print(f'g_feat_path={g_feat_path}')
            show_imgs_feats()
            index +=1
